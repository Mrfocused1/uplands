...
type CompetenceDetailsScreenProps = {
...
export function CompetenceDetailsScreen({
  fields,
  answers,
  current,
  total,
    ...
    <div className="min-h-[calc(100vh-6rem)] px-5 pb-0 pt-6 sm:px-8">
      <div className="mx-auto max-w-3xl">
        <div className="mb-8 flex flex-wrap items-center gap-3">
          ...
          <span className="text-sm font-bold uppercase text-zinc-500">{sectionLabels.competence.title}</span>
        </div>

        <p className="mb-3 font-din text-sm uppercase text-uplands-magenta">{sectionLabels.competence.subtitle}</p>
        <h2 className="font-slab text-4xl font-light leading-tight text-uplands-charcoal sm:text-5xl">Competence details</h2>
        <p className="mt-5 text-lg leading-8 text-zinc-600">Enter your CSCS and client-specific training details.</p>

        <div className="mt-9 grid gap-5 sm:grid-cols-2">
          {fields.map((field, index) => (
            <div key={field.id}>
              <label htmlFor={`competence-${field.id}`} className="mb-2 block text-base font-bold text-uplands-charcoal">
                {field.label}
              </label>
              <input
                id={`competence-${field.id}`}
                autoFocus={index === 0}
