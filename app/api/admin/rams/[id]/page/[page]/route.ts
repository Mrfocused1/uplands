...
export async function GET(_request: Request, context: { params: Promise<{ id: string; page: string }> }) {
  ...
  const localPath = getStorageProvider().getLocalPath?.({ key: document.storage_key });
  if (!localPath) return NextResponse.json({ error: "Page rendering is not available for this storage provider." }, { status: 501 });
