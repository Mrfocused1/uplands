...
import { getSubmission, loadEvidenceDocument, transformFromRow } from "@/lib/db/submissions";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const uploadedDocuments: UploadedDocument[] = await Promise.all(
    evidence.filter((document) => document.storage_path).map(async (document) => {
