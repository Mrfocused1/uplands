...
function serializePermit(row: Awaited<ReturnType<typeof listPermitsBySite>>[number]) {
  return {
    ...
    ramsDocumentId: row.rams_document_id,
    ramsDocumentTitle: row.rams_document_title,
    ramsDocumentReference: row.rams_document_reference,
    ramsDocumentRevision: row.rams_document_revision,
...
export async function POST(request: Request) {
  ...
  try {
    id = await createPermit({
      ...
      ramsDocumentId: value(body, "ramsDocumentId") || null,
