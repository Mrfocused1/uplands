 ...
 * card (~1.586:1); certificates are portrait A4 (~0.7071:1).
 */
export const CAMERA_GUIDES: Record<EvidenceType, CameraGuideConfig> = {
  ...
  firstAid: { aspectRatio: 0.7071, label: "Certificate (portrait)", hint: "Fit the certificate inside the guide" },
  smstsSssts: { aspectRatio: 0.7071, label: "Certificate (portrait)", hint: "Fit the certificate inside the guide" },
  ipaf: { aspectRatio: 0.7071, label: "Certificate (portrait)", hint: "Fit the certificate inside the guide" },
  pasma: { aspectRatio: 0.7071, label: "Certificate (portrait)", hint: "Fit the certificate inside the guide" },
};
...
export function evidenceTypeFromFieldId(fieldId: string): EvidenceType | undefined {
  switch (fieldId) {
    ...
    case "firstAiderUpload":
      return "firstAid";
    case "smstsSsstsUpload":
      return "smstsSssts";
    case "ipafUpload":
      return "ipaf";
    case "pasmaUpload":
